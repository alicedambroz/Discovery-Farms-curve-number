...
plot_rainfall_runoff <- function(df_event, event_id, site_name) {
  ...
  # Extract the Frozen status (it's the same for all rows in this event)
  frozen_status <- unique(df_event$Frozen)[1]
  
  # Create a custom title string
  plot_title <- paste0("Site: ", site_name, " | Event: ", event_id, " | Condition: ", frozen_status)
  
  # Optional: Make title RED if Frozen, BLACK if Nonfrozen (visual aid)
  ...
  g_rain <- ggplot(df_event, aes(x = Date)) +
    ...
    theme(
      ...
      plot.title = element_text(color = title_color, face = "bold") # Apply color here
    ) +
    labs(title = plot_title) # Use new title
...
# 3. LOOP
# We extract unique combinations of Site + Event. 
# We don't need 'Frozen' in this list to drive the loop, but the function will look it up.
...
```{r}
# Ensure Date is parsed correctly (adjust format if your CSV is different)
df_events$Date <- ymd_hms(df_events$Date) 
...
plot_rainfall_runoff <- function(df_event, event_id) {
  ...
  g_rain <- ggplot(df_event, aes(x = Date)) +
    ...
    labs(title = paste("Event ID:", event_id)) # Add Event ID title

  # --- Plot 2: Runoff / Volume (Bottom) ---
  g_flow <- ggplot(df_event, aes(x = Date)) +
    geom_line(aes(y = Volume_mm), color = "black", linewidth = 1) +
    scale_y_continuous(
    ...
    theme(
      axis.title.y = element_text(size = 10),
      axis.text.y = element_text(size = 8),
      axis.text.x = element_text(angle = 45, hjust = 1), # Angle x-text for readability
      plot.margin = margin(t = 0, r = 10, b = 10, l = 10) # Reduce top margin
    )

  # --- Align Axes (Crucial for visual comparison) ---
  # This aligns the plot panels vertically even if y-axis label widths differ
  g_rain_grob <- ggplotGrob(g_rain)
  g_flow_grob <- ggplotGrob(g_flow)
  ...
  # Combine: Rainfall gets 1 part height, Flow gets 2 parts
  combined_plot <- plot_grid(g_rain_grob, g_flow_grob, ncol = 1, rel_heights = c(1, 2), align = "v")
  
  return(combined_plot)
}

# 3. (Optional) Loop to create a multi-page PDF
# This is the best way to inspect many events quickly.
